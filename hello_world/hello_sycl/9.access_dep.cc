...
int main(int argc, char* argv[]) {
    ...
    // 
    // 原因在于, 即使多个 kernel 中有些是 `read A`, 有些是 `write A`, sycl runtime
    // 也并不能推导出 `read A` 依赖 `write A`, 需要用户通过 submit 来告诉 runtime
    // 针对 `A` 的 data race 是 read-write 还是 write-read.
    // 
    ...
    // 3. z=x+y
    // 
    // 编译器通过分析代码能知道 (1,3),(2,3) 有 data race (都是 read-after-write),
    // 但 (1,2) 并没有, 所以 1,2可以自由的乱序执行    
