...
class MFCC(object):
    ...
    @staticmethod
    def decode_wav(file_name, time_shift=0, noise=None):
        ...
        if time_shift != 0:
            data = _apply_time_shift(data, time_shift)
        if noise is not None:
            data = _apply_background_noise(data, noise)
        return MFCC.decode_pcm(data)
    ...
    def _apply_background_noise(data, noise):
        data += noise
        return data

    # max_shift: ms
    def _apply_time_shift(data, max_shift):
